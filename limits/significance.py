...
signal_src_mu  ='MuSigSel_v1'
signal_src_ele ='EleSigSel_v1'

rootDir = "ROOT_Significance_phd/"
...
#model_file = 'ROOTDataShape_50_0_3500_eta4/Eta4_BprimeT_LH_10_rebinned_sr_rescale.root' #significance_BprimeT_RH.root' # significance_test.root'
#model_file = 'ROOTDataShape_50_0_3500_eta4/Eta4_X53_LH_10_rebinned_sr_rescale.root' #significance_BprimeT_RH.root' # significance_test.root'
model_file = 'ROOTDataShape_50_0_3500_eta4/Eta4_X53_LH_rebinned_sr_rescale.root' #significance_BprimeT_RH.root' # significance_test.root'
...
result_file = open(signal+width+'_'+chirality+'_significance_results.txt','w+')
...
for m,mass in enumerate(masses):
    ...
    print 'sculpted file' 
    print model_file, signal, '_'+mass+'_',
    
    single_mass_no_unc =  rename_process(single_mass_no_unc, signal+'_'+mass+width+'_'+chirality,"Signal_forward_"+mass)
    single_mass_renamed = rename_process(single_mass_unc   , signal+'_'+mass+width+'_'+chirality,"Signal_forward_"+mass)
    ...
    central_sig = rootDir+signal+mass+width+'_'+chirality+'.root'
    mu_central_sig = central_sig.replace('.root','_mu.root')
    ele_central_sig = central_sig.replace('.root','_ele.root')
    
    call(['./../bin/rootfilecreator', signal.replace('X','X53')+'-'+mass+width.replace('Width_','Width-')+'_'+chirality, mu_central_sig, release+signal_src_mu , 'Mu' ,"","","central"])
    call(['./../bin/rootfilecreator', signal.replace('X','X53')+'-'+mass+width.replace('Width_','Width-')+'_'+chirality, ele_central_sig, release+signal_src_ele, 'Ele',"","","central"])

    #exit(0)
    call(['rm '+central_sig],shell=True)
    call(['hadd '+central_sig+' '+mu_central_sig+' '+ele_central_sig],shell=True)
    central_sig = rename_process(central_sig,signal.replace('X','X53')+'_'+mass+width+'_'+chirality,"Signal_central_"+mass)
	...
	print >> result_file, 'z value substraction not possible factor 1 going to retry with 0.1'
        #exit(0)
    try:
	...
	print >> result_file, 'z value substraction not possible factor 0.1'
